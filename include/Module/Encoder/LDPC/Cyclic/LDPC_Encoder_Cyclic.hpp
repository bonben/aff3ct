...
protected:
	...
	virtual std::vector<B> _CSRAA(const int Zc, std::vector<B> Gen, const B * vect, const int K, const int N);
	virtual void _MultiplyAdd(std::vector<B>&v, int k, std::vector<B>&r);
};
}
}
